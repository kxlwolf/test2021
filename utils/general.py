...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.5, merge=False, classes=None, agnostic=False):
    ...
    t = time.time()
    output = [None] * prediction.shape[0] #prediction.shape[0]代表的是检测图片的个数，一般为1
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Batched NMS
        """c is an offset according to the different class of the predictive box, so that the predictive box coordinates of each class will be separated, 
        and the prediction boxes of different class will not be removed when we do NMS. max_ Wh is a coefficient that large enough to separate the box 
        coordinates of each class."""
        c = x[:, 5:6] * (0 if agnostic else max_wh)  # classes, 
        boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), scores
        i = torch.ops.torchvision.nms(boxes, scores, iou_thres)
        #print("Batched NMS i.shape: ", i.shape)
        if i.shape[0] > max_det:  # limit detections
            i = i[:max_det]
        """在进行NMS时，是否通过合并方式获得预测框"""
