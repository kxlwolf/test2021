...
#import test
import evaluate  # import test.py to get mAP after each epoch
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        # print("ckpt: ", ckpt)
        print("ckpt: ['epoch'], ['best_fitness'], ['training_results'], ['optimizer']: ", ckpt['epoch'], ckpt['best_fitness'], ckpt['training_results'], ckpt['optimizer'])
        ...
        exclude = (['anchor'] if opt.cfg or hyp.get('anchors') else [])  # exclude keys, 以opt.cfg中的anchor为主，其次是hyp中的
    ...
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lf) #每个epochs具有一个不同的学习率
    ...
    imgsz, imgsz_test = [check_img_size(x, gs) for x in opt.img_size]  # verify imgsz are gs-multiples  #检查输入图片分辨率是gs=32的整数倍
    print("imgsz: ", imgsz)
    ...
    # Start training
    t0 = time.time()
    n_warmup = max(round(hyp['warmup_epochs'] * nb), 1e3)  # number of warmup iterations, max(3 epochs, 1k iterations)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= n_warmup:
                ...
                accumulate = max(1, np.interp(ni, [0, n_warmup], [1, nbs / total_batch_size]).round())
                for j, x in enumerate(optimizer.param_groups):
                    # bias lr falls from 0.1 to lr0, all other lrs rise from 0.0 to lr0
                    x['lr'] = np.interp(ni, [0, n_warmup], [hyp['warmup_bias_lr'] if j == 2 else 0.0, x['initial_lr'] * lf(epoch)])
                    if 'momentum' in x:
                        x['momentum'] = np.interp(ni, [0, n_warmup], [hyp['warmup_momentum'], hyp['momentum']])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./weights/download/yolov5l.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='./cfg/download/yolov5l.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/seacoast.yaml', help='data.yaml path') #数据集配置文件，路径，类名
    parser.add_argument('--hyp', type=str, default='data/hyp/hyp.scratch.yaml', help='hyperparameters path') #超参数文件
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch-size', type=int, default=32, help='total batch size for all GPUs')
    parser.add_argument('--img-size', nargs='+', type=int, default=[416, 416], help='[train, test] image sizes')
    
    parser.add_argument('--rect', action='store_true', help='rectangular training') #是否采用矩阵训练
    parser.add_argument('--noautoanchor', action='store_true', help='disable autoanchor check') #不自动调整anchor
    parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters') #是否进行超参数进化
    parser.add_argument('--multi-scale', action='store_true', help='vary img-size +/- 50%%')
    parser.add_argument('--single-cls', action='store_true', help='train as single-class dataset')
    parser.add_argument('--adam', action='store_true', help='use torch.optim.Adam() optimizer')
    parser.add_argument('--image-weights', action='store_true', help='use weighted image selection for training')
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training') #是否提前缓存图片到内存

    #parser.add_argument('--resume', nargs='?', const=True, default='./runs/vehicle/exp1/weights/last.pt', help='resume most recent training') #接着上次训练结果训练
    parser.add_argument('--resume', nargs='?', const=True, default=False)
    parser.add_argument('--nosave', action='store_true', help='only save final checkpoint')
    parser.add_argument('--notest', action='store_true', help='only test final epoch')
    parser.add_argument('--name', default='', help='renames results.txt to results_name.txt if supplied')
    parser.add_argument('--logdir', type=str, default='runs/', help='logging directory') #存放日志的目录
    ...
    parser.add_argument('--bucket', type=str, default='', help='gsutil bucket') #谷歌云盘bucker,一般用不到
